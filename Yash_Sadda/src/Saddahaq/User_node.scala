    ...
    else if(item_type.toLowerCase().equals("ev"))
    {
      if(user_node != null && user_node.hasRelationship("Belongs_To_Location"))
      {
	        ...
	        ret = (loc_events ::: index.query("id", "*" ).iterator().asScala.toList.sortBy(-_.getProperty("event_date_time_closing").toString().toInt))
	        		...
	        		.map(x => JSONObject(l.zip(
	        		    List(
	        		        x.getProperty("event_date_time_closing").toString(),
      ...
      {
            ret = index.query("id", "*" ).iterator().asScala.toList.filter(x => x.getRelationships("Belongs_To_Event_Category").asScala.toList.map(_.getOtherNode(x)).map(y => y.getProperty("name")).filterNot(x => x.equals("all")).size > 0).filter( x => x.getProperty("space").toString.toInt == 0).sortBy(-_.getProperty("event_date_time_closing").toString().toInt).slice(prev_cnt,(prev_cnt+count)).map(x => JSONObject(
                l.zip(List(
                		x.getProperty("event_date_time_closing").toString(),
                		x.getRelationships("Comment_To_Event").asScala.toList.map(y=>y.getOtherNode(x).getSingleRelationship("Comment_Written_By",Direction.OUTGOING).getEndNode()).distinct.size,
                		1,
...
/*    System.out.println(get_spaces("user100","s",1,0));
    System.out.println(get_spaces("user100","s",2,0));
    System.out.println(get_spaces("user100","s",2,1));
    System.out.println(get_spaces("user100","s",10,0)); */
    
    /*System.out.println(get_all_items("ar","user100",10,0));
    //System.out.println(get_all_items("ev","user100",1,0));
    System.out.println(get_all_items("ev","user100",1,1));
    System.out.println(get_all_items("ev","user100",1,2));
    System.out.println(get_all_items("ev","user100",1,3));
